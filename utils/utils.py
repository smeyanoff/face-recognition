...
class ChoosePersons:
    def __init__(
        ...
        self.person_photos = {
            person: np.random.choice(
                os.listdir(os.path.join(self.photo_path, person)), size=photo_size
            )
    ...
    def random_choose_person_photos(self):
        ...
        proba_persons = [
            (1 - config.FROAD_PROBA)
            if x == person1
            else config.FROAD_PROBA / (len(self.persons) - 1)
            for x in self.persons
        ]

        person2 = np.random.choice(a=self.persons, p=proba_persons)
        ...
        person1_path = os.path.join(self.photo_path, person1)
        person2_path = os.path.join(self.photo_path, person2)

        # random choose the photo of each person
        photo1 = np.random.choice(self.person_photos[person1])  # choose random
        photo2 = np.random.choice(self.person_photos[person2])  # choose random
